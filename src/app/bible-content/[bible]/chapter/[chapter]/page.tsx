...
export default async function BibleContent({
  params,
}: Readonly<{ params: { bible: string; chapter: string } }>) {
  const chapters: Chapter[] =
    baseUrl === ""
      ? []
      : await (await fetch(`${baseUrl}/bible/${params.bible}/chapters`)).json();
  const book_contents: BookContent[] =
    baseUrl === ""
      ? []
      : await (
          await fetch(
            `${baseUrl}/bible/books/${params.bible}/chapter/${params.chapter}/content`
          )
        ).json();
      ...
      <div className="grid grid-cols-12 gap-1 pb-5">
        ...
        <div className="col-span-12 md:col-span-8">
          <div className="grid grid-cols-10 gap-1 pb-4">
            <Link
              href={"/"}
              className="py-1 text-center shadow-md text-lg font-bold bg-slate-100 rounded-lg hover:bg-red-200 hover:text-white"
            >
              Back
            </Link>
            <ChapterComponent
          ...
          <BookChapter
            name={selectedChapter?.book.name}
          ...
          <div className="flex  md:space-x-96 pt-3 text-2xl">
            {selectedChapter?.chapter_id !== 1 ? (
              <Link
                href={`/bible-content/${params.bible}/chapter/${
                  Number(selectedChapter?.chapter_id ?? 0) - 1
                }`}
                className="rounded-lg mr-3 hover:bg-sky-300 py-1 px-5  bg-sky-300  text-white"
              >
                Shemppuwaa {Number(selectedChapter?.chapter_id ?? 0) - 1}
              </Link>
            ) : (
              <span className="py-1 px-10 mr-3"></span>
            )}
            {chapters.length !== Number(selectedChapter?.chapter_id ?? 0) && (
              <Link
                href={`/bible-content/${params.bible}/chapter/${
                  (selectedChapter?.chapter_id ?? 0) + 1
                }`}
                className="rounded-lg  hover:bg-sky-300 py-1 px-5 bg-sky-300  text-white"
              >
                Shemppuwaa {Number(selectedChapter?.chapter_id ?? 0) + 1}
              </Link>
            )}
